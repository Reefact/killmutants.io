#!/bin/sh
# Validate a commit message against killmutants.io's commit convention.
#
...
# CONTRIBUTING.md; this script only mirrors them. Adapted from just-dummies'
# tools/commit-lint/lint-commit-message.sh — trimmed for a site with no
# per-package release-train tooling, so (unlike there) a scope is never
# required, and its value isn't restricted to a closed list.
#
# Usage:
...
# lands pull requests by rebase, so a placeholder merged before its autosquash
# would otherwise land, unlinted, in protected history.
case "$subject" in
  'fixup! '*|'squash! '*|'amend! '*)
    if [ "$strict" = 0 ]; then
      exit 0
...
if [ -z "$subject" ]; then
  ...
  # canonical shape: <type>[(<scope>)][!]: <lowercase description>
  # (a single scope word — letters, digits, hyphens; this repo has no
  # per-package release-train tooling, so unlike just-dummies a scope is
  # never required and isn't checked against a closed list.)
  if printf '%s' "$subject" | grep -Eq "^(${TYPES})(\([a-z0-9-]+\))?!?: [a-z]"; then
    : # well-formed header, nothing more to check here
  else
    ...
    if printf '%s' "$subject" | grep -Eq '^[a-zA-Z]+\([^)]*[^a-z0-9-][^)]*\)'; then
      err "the scope must be a single lowercase word (letters, digits, hyphens only)"
    fi
